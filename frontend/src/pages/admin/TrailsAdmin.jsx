...
const EMPTY_FORM = {
    titlu: '', descriere: '', durataEstimata: 120, oras: '',
    imagineUrl: '', activ: true, locatiiValide: []
};
...
export default function TrailsAdmin() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (trRes.data.success) setTrails(trRes.data.data);
    ...
    const handleOpenModal = (trail = null) => {
        if (trail) {
            ...
            setFormData({
                titlu: trail.titlu || '', descriere: trail.descriere || '',
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            setFormError(err.response?.data?.message || 'Eroare la salvarea traseului.');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(`${API}/api/trails/admin/${confirmTarget}`, { withCredentials: true });
            fetchData();
        } catch (err) {
    ...
    const removeLocationFromTrail = (idx) => {
        setFormData(prev => ({ ...prev, locatiiValide: prev.locatiiValide.filter((_, i) => i !== idx) }));
    };
    ...
    return (
        <div className="admin-page rewards-admin">
                    ...
                    <tbody>
                        {trails.length === 0 ? (
                            ...
                            <tr key={t.id} className={!t.activ ? 'row-inactive' : ''}>
                                ...
                                <td><strong>{t.locatii?.length || 0}</strong> locații</td>
                                <td>
                                    {t.activ ? (
                                ...
                                <td>
                                    <div className="action-buttons">
                                        ...
                                        <button className="btn-icon delete" onClick={() => setConfirmTarget(t.id)} title="Șterge">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </td>
            ...
            <FormModal
                show={showModal}
                title={editingId ? 'Editează Traseu Cultural' : 'Creează Traseu Cultural'}
            ...
            >
                <div className="form-row">
                    <div className="form-group flex-1" style={{ flex: 2 }}>
                        ...
                        <input required type="text" value={formData.titlu} onChange={e => set('titlu', e.target.value)} placeholder="ex: Weekend la Castele" />
                    </div>
                    <div className="form-group flex-1">
                        <label>Oraș (sau Regiune) *</label>
                        <input required type="text" value={formData.oras} onChange={e => set('oras', e.target.value)} placeholder="ex: Brașov" />
                    </div>
                </div>
                ...
                <div className="form-row">
                    <div className="form-group flex-1">
                        ...
                        <input required type="number" min="1" value={formData.durataEstimata} onChange={e => set('durataEstimata', parseInt(e.target.value) || 0)} />
                    </div>
                    <div className="form-group flex-1">
                        <label>URL Imagine Cover</label>
                        <input type="url" value={formData.imagineUrl} onChange={e => set('imagineUrl', e.target.value)} placeholder="https://..." />
                    </div>
                </div>
            ...
            <ConfirmDialog
                show={!!confirmTarget}
                title="Ștergere Traseu"
                message="Ești sigur că vrei să ștergi acest traseu cultural?"
